...
`$ rails generate model Relationship follwer_id:integer followed_id:integer`
...
at the a follow or unfollow: the Relationship.
...
use `:follower_id` to show `active_relationships` and `:followed_id` as a
...
Now let's create an association within the `Relationship` model with a 
few validations:
...
| GET          | users/:id/followers  | followers | followers_user_path(:id) |
| CREATE       | relationships        | follow    | relationships_path       |
| DELETE       | relationships/:id    | unfollow  | relationship_path(:id)   |

Starting with the GETs, in order to acheive these kinds of nested routes, 
we'll need to manipulate the resources routes for `:users`.
...
`RelationshipsController`. In this controller, we'll limit all actions 
by checking for a logged in user.
